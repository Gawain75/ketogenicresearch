const setLang = (lang) => {
  ...
  if (typeof renderLatestEvidence === 'function') {
    renderLatestEvidence();
  }

  if (typeof updateLibraryCounters === 'function') {
    updateLibraryCounters();
  }

  if (typeof syncLiteratureUpdateDate === 'function') {
    ...
    rows.map(p => {
      ...
      const areas =
        (
          ...
          .map(
            a =>
              `<span class="latest-area-chip">${krEscapeHtml(a)}</span>`
          )
          .join('');
      ...
      const evidenceBadge =
        evidenceLabel
          ? `<span class="latest-evidence-chip">${krEscapeHtml(
              evidenceLabel
            )}</span>`
          : '';
