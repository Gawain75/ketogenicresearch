...
def main():
    ...
    if not pending:
        print("All article drafts are already on generator version 3.8.")
        return
    ...
    for index, (path, pmid) in enumerate(pending, start=1):
        ...
        try:
            ...
            if str(check.get("verdict", "")).upper() != "PASS":
                ...
                changed += 1
                print(f"[{index}] Replaced with V3.8: {path.name}")
    ...
    print(f"Articles still requiring V3.8 regeneration: {remaining}")
